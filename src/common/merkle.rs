    ...
    fn compute(&mut self) {
        ...
        loop {
            ...
            for i in 0..(nodes_end_index - nodes_start_index) / 2 {
                ...
                self.nodes.push(Self::pairwise_hash(
                    &self.nodes[base_index],
                    &self.nodes[base_index + 1],
                ));
            }
...
mod test {
    ...
    fn test_merkle_tree_with_odd_elements() {
        ...
        assert!(
            !mt.is_root_valid(),
            "Root is flagged as valid after tree modification"
        );
        assert_eq!(
        ...
        assert!(
            mt.is_root_valid(),
            "Root is flagged as invalid after root calculation"
        );
        assert_eq!(
            mt.nodes.len(),
            values.len() * 2 - 1,
            "Merkle Tree has more nodes than expected"
        );
        for i in 0..leaves_to_prove.len() {
            ...
            for j in 0..std::cmp::max(expected_proofs[i].len(), proof.len()) {
                assert_eq!(
                    proof[j], expected_proofs[i][j],
                    "Proofs for test leaf {} differ at index {}",
                    i, j
                );
            }
            assert!(
                mt.verify(&leaves_to_prove[i], &proof),
    ...
    fn test_merkle_tree_with_one_element() {
        let values = vec!["a"];
        let expected_root =
            Bytes32::try_from("3ac225168df54212a25c1c01fd35bebfea408fdac2e31ddd6f80a4bbf9a5f1cb")
                .unwrap();
        let mut mt = MerkleTreeAppendOnly::<Keccak256>::new();
        values.iter().for_each(|v| {
            mt.append(v);
        ...
        assert!(
            mt.is_root_valid(),
            "Root is flagged as invalid with no tree modification"
        );
        assert_eq!(
            mt.nodes.len(),
            values.len() * 2 - 1,
    ...
    fn test_merkle_root_with_no_elements() {
        ...
        assert!(
            !mt.is_root_valid(),
            "Root is flagged as valid for empty tree"
        );
        assert!(mt.root().is_none());
        assert!(
            !mt.is_root_valid(),
            "Root is flagged as valid for empty tree after root request"
        );
    }
}
