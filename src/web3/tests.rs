...
mod tests {
    ...
    use crate::{
        ...
        web3::{
            ethereum::{new_ethereum_timestamper_from_devnode, EthereumTimestamper},
            stub::EthereumStub,
    ...
    async fn test_register_device() {
        ...
        match timestamper.register_device(&device).await {
            ...
            Ok(_) => {
                ...
                assert!(
                    gotten_device.is_ok(),
                    "Could not get Device after registration"
                );
                let gotten_device = gotten_device.unwrap();
                assert_eq!(
                    gotten_device.id, device.id,
                    "Registered a different ID than supplied"
                );
                assert_eq!(
                    gotten_device.pk.0, device.pk.0,
                    "Registered a different Public Key than supplied"
                );
            }
        }
    }
                    ...
                    match merkle_receipt {
                        MerkleTreeOZReceipt::Tree(ref mut mt) => {
                            if let Some(root) = mt.root() {
                                let gotten_merkle_root = timestamper
                                    .get_dataset(dataset.id, device.id)
                                    .await
                                    .unwrap();
